package OAuth2TokenVendingServer::Web::View;
use strict;
use warnings;
use utf8;
use Carp ();
...
sub make_instance {
    my ($class, $context) = @_;
    Carp::croak("Usage: OAuth2TokenVendingServer::Web::View->make_instance(\$context_class)") if @_!=2;
    ...
    unless (exists $view_conf->{path}) {
        ...
        if ( -d $tmpl_path ) {
            ...
            my $share_tmpl_path = eval { File::Spec->catdir(File::ShareDir::dist_dir('OAuth2TokenVendingServer'), 'tmpl') };
            if ($share_tmpl_path) {
    ...
    my $view = Text::Xslate->new(+{
        ...
        'module'   => [
            ...
            'OAuth2TokenVendingServer::Web::ViewFunctions',
        ],
        'function' => {
        },
